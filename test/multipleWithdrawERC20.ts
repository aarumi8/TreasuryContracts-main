...
describe("Vault ERC20 with multiple tokens", function () {
  ...
  let vault: Vault;
  let erc20Primary: ERC20WithMint;
  ...
  beforeEach(async function () {
    ...
    erc20Primary = await ERC20WithMint.connect(primaryOwner).deploy();
  ...
  it("empty tokens list should be reverted", async function() {
    await expect(vault.connect(user).withdrawMultipleTokensByERC20([], 50, user.address))
      .to
      .be
  ...
  it("not sorted tokens should be reverted", async function() {
    await expect(vault.connect(user).withdrawMultipleTokensByERC20([
  ...
  it("repeated tokens should be reverted", async function() {
    await expect(vault.connect(user).withdrawMultipleTokensByERC20([erc20RewardTokens[0].address, erc20RewardTokens[0].address], 50, user.address))
      .to
      .be
      .revertedWith("tokens should be sorted and unique");
  });

  it("can't create second vault for primary token", async function() {
    await expect(vaultFactory.connect(factoryCreator).createNewVault(erc20Primary.address, true))
      .to
      .be
      .revertedWith("vault for this token already exists");
  });
  ...
  it("withdraw only eth", async function() {
    ...
    // withdraw from vault
    const tx = await vault.connect(user).withdrawMultipleTokensByERC20([ETH_ADDRESS], 50, user.address);

    expect(await erc20Primary.balanceOf(user.address)).to.equal(0);
    expect(await erc20Primary.balanceOf(vault.address)).to.equal(0);
  ...
  it("withdraw two erc20", async function() {
    // withdraw from vault
    await vault.connect(user).withdrawMultipleTokensByERC20([erc20RewardTokens[0].address, erc20RewardTokens[1].address].sort(), 50, user.address);
  ...
  it("withdraw two erc20 and eth", async function() {
    ...
    // withdraw from vault
    const tx = await vault.connect(user).withdrawMultipleTokensByERC20([erc20RewardTokens[0].address, erc20RewardTokens[1].address, ETH_ADDRESS].sort(), 50, user.address);
